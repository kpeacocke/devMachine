...
if (Get-Command "Microsoft.PowerShell.PSConsoleReadLine" -ErrorAction SilentlyContinue) {
    try {
...
try {
    ...
    if (Test-Path $vscodeSettingsPath) {
        ...
        if (-not $vscodeSettings.'terminal.integrated.shellIntegration.enabled') {
            $vscodeSettings | Add-Member -MemberType NoteProperty -Name 'terminal.integrated.shellIntegration.enabled' -Value $true -Force
        }
        if (-not $vscodeSettings.'terminal.integrated.shellIntegration.decorationsEnabled') {
            $vscodeSettings | Add-Member -MemberType NoteProperty -Name 'terminal.integrated.shellIntegration.decorationsEnabled' -Value $true -Force
        }
        if (-not $vscodeSettings.'terminal.integrated.shellIntegration.showExitStatus') {
            $vscodeSettings | Add-Member -MemberType NoteProperty -Name 'terminal.integrated.shellIntegration.showExitStatus' -Value $true -Force
        }
        if (-not $vscodeSettings.'powershell.powerShellDefaultVersion') {
            $vscodeSettings | Add-Member -MemberType NoteProperty -Name 'powershell.powerShellDefaultVersion' -Value 'PowerShell' -Force
        }
...
Write-Host "💡 Shell integration features available in VS Code terminals!" -ForegroundColor Yellow
