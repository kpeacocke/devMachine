...
.DESCRIPTION
    ...
    C:\DevCache. The user's C:\Users\<user>\code path is a junction to
    C:\DevCache\code.

    The script uses Windows' reported supported C: shrink boundary and requires
    at least 30% free space on C: after the operation. Existing undersized
    DevCache/DevCode volumes are never deleted automatically.

    Use -WhatIf for a non-destructive preflight. SupportsShouldProcess provides
    the common -WhatIf parameter; this script does not declare another one.
#>
...
function Get-VolumePartition {
    ...
    foreach ($p in @(Get-Partition -ErrorAction Stop)) {
        ...
        if ($v -and $v.FileSystemLabel -eq $Volume.FileSystemLabel -and $v.Size -eq $Volume.Size) {
            return $p
        }
    }
    return $null
...
function Test-DevDrive {
    ...
    $out = & $FsUtil devdrv query $Path 2>&1
    if ($LASTEXITCODE -ne 0) { return $false }
    return (($out | Out-String) -match '(?i)developer volume')
}

function Trust-DevDrive {
    param([Parameter(Mandatory)][string]$Path)
    $out = & $FsUtil devdrv trust $Path 2>&1
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to trust Dev Drive $Path (exit code $LASTEXITCODE). $($out | Out-String)"
    }
}
...
function Ensure-CodeJunction {
    if (Test-Path -LiteralPath $CodePath) {
        ...
        if ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) {
            ...
            throw "$CodePath is an existing reparse point with target '$target'. Refusing to change it."
        }
        $items = @(Get-ChildItem -LiteralPath $CodePath -Force -ErrorAction Stop)
        if ($items.Count -gt 0) {
            throw "$CodePath exists and contains data. Refusing to replace it with a junction."
        }
...
function Ensure-MountPoint {
    ...
    if (Test-Path -LiteralPath $MountPoint) {
        ...
        $items = @(Get-ChildItem -LiteralPath $MountPoint -Force -ErrorAction Stop)
        if ($items.Count -gt 0) { throw "$MountPoint exists and is not an empty mount-point directory." }
    }
    else {
...
# Never silently consume or destroy the old failed layout.
$legacyCode = Get-LabeledVolume 'DevCode'
$cache = Get-LabeledVolume 'DevCache'
if ($legacyCode) {
    throw "Legacy DevCode volume detected ($([math]::Round($legacyCode.Size / 1GB,1)) GB). Remove it explicitly before continuing."
}
if ($cache -and $cache.Size -lt ([uint64]$MinimumGB * 1GB)) {
    throw "Undersized legacy DevCache volume detected ($([math]::Round($cache.Size / 1GB,1)) GB). Remove it explicitly before continuing."
}

# Idempotent path: an existing valid Dev Drive is reused.
if ($cache) {
...
# We need a contiguous gap immediately after C because New-Partition will consume
# a suitable free extent; shrinking C creates the required contiguous extent.
$parts = @(Get-Partition -DiskNumber $disk.Number -ErrorAction Stop | Sort-Object Offset)
$cEnd = [uint64]$c.Offset + [uint64]$c.Size
$next = $parts | Where-Object { [uint64]$_.Offset -gt $cEnd } | Select-Object -First 1
$gap = if ($next) { [uint64]$next.Offset - $cEnd } else { [uint64]$disk.Size - $cEnd }

$requested = [uint64]$DevDriveGB * 1GB
$fromC = [uint64][Math]::Max([int64]0, [int64]$requested - [int64]$gap)
$projectedCSize = [uint64]$c.Size - $fromC
$projectedFree = [uint64]$cvol.SizeRemaining - $fromC
$projectedFreePct = if ($projectedCSize) { ($projectedFree / $projectedCSize) * 100 } else { 0 }
...
Write-Host "   Projected C: free:       $([math]::Round($projectedFreePct,1))%" -ForegroundColor Gray

if ($fromC -gt $maxShrink) {
    throw "Cannot create a $DevDriveGB GB Dev Drive: required C: shrink $([math]::Round($fromC/1GB,1)) GB exceeds Windows maximum $([math]::Floor($maxShrink/1GB)) GB."
}
if ($projectedFreePct -lt 30) {
    throw "Refusing to shrink C: below 30% free. Projected free: $([math]::Round($projectedFreePct,1))%."
}

if (-not $PSCmdlet.ShouldProcess("Disk $($disk.Number)", "Create $DevDriveGB GB Dev Drive mounted at $MountPoint")) {
    return
}
...
if ($fromC -gt 0) {
    ...
    Write-Host "   ✅ C: resized" -ForegroundColor Green
}
...
$letter = $new.DriveLetter

try {
    Format-Volume -DriveLetter $letter -FileSystem ReFS -NewFileSystemLabel 'DevCache' -DevDrive -Confirm:$false -ErrorAction Stop | Out-Null
    Trust-DevDrive -Path "$letter`:\"
    Ensure-MountPoint -Partition (Get-Partition -DriveLetter $letter -ErrorAction Stop)
    Remove-PartitionAccessPath -DiskNumber $new.DiskNumber -PartitionNumber $new.PartitionNumber -AccessPath "$letter`:\" -ErrorAction Stop
    Trust-DevDrive -Path $MountPoint
...
if ($final.Size -lt ([uint64]$MinimumGB * 1GB)) { throw 'Created DevCache is below the 50 GB Dev Drive minimum.' }
if (-not (Test-DevDrive -Path $MountPoint)) { throw 'C:\DevCache is not recognised as a Dev Drive.' }
if (-not (Test-Path -LiteralPath $CodePath)) { throw "Code path was not created: $CodePath" }

Write-Host "   ✅ DevCache: $([math]::Round($final.Size/1GB,1)) GB" -ForegroundColor Green
Write-Host "   ✅ Mount: $MountPoint" -ForegroundColor Green
Write-Host "   ✅ Code: $CodePath → $CodeTarget" -ForegroundColor Green
Write-Host "   ✅ Trusted Dev Drive; Defender Performance Mode remains enabled" -ForegroundColor Green
